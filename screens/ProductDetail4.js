...
const ProductDetail4 = () => {
  ...
  const handleAddToCart = () => {
    // Kiểm tra nếu sản phẩm có màu sắc và chưa chọn màu
    ...
    setCart((prevCart) => {
      ...
      if (existingItem) {
        ...
        // Nếu sản phẩm chưa có, thêm sản phẩm mới với giá và màu đã chọn
        return [
          ...prevCart,
          {
            ...product,
            quantity,
            color: selectedColor, // Lưu thông tin màu đã chọn
          },
        ];
      }
    });
  ...
  const calculateTotalPrice = () => {
    return (product.price * quantity).toFixed(2); // Tính tổng giá theo số lượng
  };
...
{product.color && product.color.length > 0 ? (
  ...
  <Text style={styles.noColorText}>This product has no color yet.</Text>
)}
        ...
        <View style={styles.infoContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Review4', { product, relatedProducts })}>
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 12, backgroundColor: '#fcecf8' },
  ...
  selectedColor: { borderColor: '#7A8B8B', borderWidth: 3 },
  quantityContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',

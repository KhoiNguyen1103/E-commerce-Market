  ...
  setCart((prevCart) => {
    const existingItem = prevCart.find(
      (item) =>
        ...
        item.color === selectedColor // Thêm điều kiện kiểm tra màu
    );
    ...
    if (existingItem) {
      ...
      // Nếu sản phẩm chưa có, thêm sản phẩm mới với size, giá và màu đã chọn
      return [
        ...prevCart,
        {
          ...product,
...
<View style={styles.sizeContainer}>
  ...
  onPress={() => navigation.navigate('ReviewScreen2', { product, relatedProducts })}
>
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 12, backgroundColor: '#f4fcfc' },
